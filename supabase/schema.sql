...
alter table ride_requests add column if not exists email text not null default '';
alter table ride_requests add column if not exists stops jsonb not null default '[]'::jsonb;
alter table ride_requests add column if not exists payment_method text not null default 'card';
alter table ride_requests add column if not exists fare_subtotal_cents int;
alter table ride_requests add column if not exists surcharge_percent numeric not null default 0;
alter table ride_requests add column if not exists pickup_lat numeric;
alter table ride_requests add column if not exists pickup_lng numeric;
alter table ride_requests add column if not exists ride_tier text not null default 'standard';
alter table drivers add column if not exists tier text not null default 'standard';
alter table drivers add column if not exists latitude numeric;
alter table drivers add column if not exists longitude numeric;
alter table drivers add column if not exists location_accuracy numeric;
alter table drivers add column if not exists location_sharing boolean not null default false;
alter table drivers add column if not exists last_location_at timestamptz;
alter table drivers add column if not exists last_offer_at timestamptz;
create table if not exists ride_offers(id uuid primary key default gen_random_uuid(),created_at timestamptz not null default now(),ride_request_id uuid not null references ride_requests(id) on delete cascade,driver_id uuid not null references drivers(id) on delete cascade,priority int not null default 1,status text not null default 'Offered',offered_at timestamptz not null default now(),responded_at timestamptz,unique(ride_request_id,driver_id));
alter table ride_requests enable row level security;alter table driver_applications enable row level security;alter table profiles enable row level security;alter table drivers enable row level security;alter table ride_offers enable row level security;
...
create index if not exists drivers_dispatch_idx on drivers(status,tier,last_location_at desc);
create index if not exists ride_offers_driver_idx on ride_offers(driver_id,status,priority);
