...
export async function POST(req: Request) {
  try {
    ...
    if (input.when === 'now' || !input.date || !input.time) {
    ...
    const instant = input.when === 'now';
    const notesPrefix = instant ? '[INSTANT HOP] ' : String(input.notes || '').includes('[SCHEDULED]') ? '' : '[SCHEDULED] ';
    const row = {
      passenger_id: account.user.id,
      ...
      notes: `${notesPrefix}${String(input.notes || '').trim()}`.replace(/^(\[INSTANT HOP\]\s*){2,}/, '[INSTANT HOP] ').replace(/^(\[SCHEDULED\]\s*){2,}/, '[SCHEDULED] ').slice(0, 1500),
      fare_estimate_cents: fare?.totalCents || 0,
      fare_subtotal_cents: fare?.subtotalCents || null,
      surcharge_percent: fare?.surchargePercent || 0,
      distance_miles: metrics?.distanceMiles ?? null,
      duration_minutes: metrics?.durationMinutes ?? null,
      pickup_lat: metrics?.pickupLat ?? null,
      pickup_lng: metrics?.pickupLng ?? null,
      ride_tier: fare?.rideTier || 'unquoted',
      payment_method: paymentMethod,
      status: 'New',
      fare_locked: false,
      payment_status: paymentMethod === 'cash' ? 'cash_due' : 'unpaid',
    ...
    return NextResponse.json({id: data.id, fareEstimateCents: data.fare_estimate_cents, distanceMiles: data.distance_miles, rideTier: data.ride_tier, quotePending: !fare, when: instant ? 'now' : 'scheduled'}, {status: 201});
  } catch (error) {
...
export async function PATCH(req: Request) {
  ...
  try {
    ...
    if (input.assignedDriverId === null || typeof input.assignedDriverId === 'string') {
      patch.assigned_driver_id = input.assignedDriverId || null;
      if (input.assignedDriverId) patch.status = 'Assigned';
    }
